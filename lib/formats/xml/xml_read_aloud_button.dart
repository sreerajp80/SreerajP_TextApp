...
/// A "Read aloud" toggle for an XML document (task 5.5, English).
///
/// Mirrors the other formats' buttons: it asks the [TtsService] whether English
/// can be spoken and never shows a dead button — while the engine is
/// `unavailable` it renders nothing. When ready it reads the document text and
/// flips to "Stop".
class XmlReadAloudButton extends ConsumerStatefulWidget {
  final XmlDocumentSession session;
  const XmlReadAloudButton({super.key, required this.session});

  @override
  ConsumerState<XmlReadAloudButton> createState() => _XmlReadAloudButtonState();
}

class _XmlReadAloudButtonState extends ConsumerState<XmlReadAloudButton> {
