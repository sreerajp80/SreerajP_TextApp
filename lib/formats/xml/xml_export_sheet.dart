...
/// Lets the user pick an export target for an XML document, runs the conversion,
/// then offers to share or save the produced file (task 9.6).
Future<void> showXmlExportSheet(
  BuildContext context,
  XmlDocumentSession session,
  XmlOutputActions actions,
  ...
  final target = await showModalBottomSheet<ExportTarget>(
    ...
    builder: (context) => _ExportTargetList(
      targets: export.supportedTargets(XmlOutputActions.formatId),
    ),
  );
