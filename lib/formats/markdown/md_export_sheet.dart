...
/// Lets the user pick an export target for a Markdown document, runs the
/// conversion, then offers to share or save the produced file (task 6.5).
Future<void> showMdExportSheet(
  BuildContext context,
  MdDocumentSession session,
  MdOutputActions actions,
  ...
  final target = await showModalBottomSheet<ExportTarget>(
    ...
    builder: (context) => _ExportTargetList(
      targets: export.supportedTargets(MdOutputActions.formatId),
    ),
  );
