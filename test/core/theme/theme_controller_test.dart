...
void main() {
  ...
  test('hydrates from stored values', () async {
    ...
    await store.setPlainString(
      ThemeSettings.modeKey,
      AppThemeMode.sepia.prefValue,
    );
  ...
  test('font scale is clamped to a safe range', () async {
    ...
    expect(
      container.read(themeControllerProvider).fontScale,
  ...
  test(
    'setMalayalamFontFamily(null) clears state and removes the key',
    () async {
      final store = await inMemoryKeyValueStore();
      await store.setPlainString(
  ...
  test('a corrupt stored mode falls back to system', () async {
    ...
    final container = containerWith(store);
    expect(container.read(themeControllerProvider).mode, AppThemeMode.system);
  });
}
