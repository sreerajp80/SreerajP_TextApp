...
void main() {
  ...
  testWidgets(
    'PinchToZoomArea ignores horizontal pinch gestures but accepts vertical ones',
    (tester) async {
      final store = await inMemoryKeyValueStore();
      ...
      final TestGesture gestureH1 = await tester.startGesture(
        const Offset(100, 100),
      );
      final TestGesture gestureH2 = await tester.startGesture(
        const Offset(200, 200),
      );
      await tester.pump();
      ...
      // Start with two pointers at (100, 100) and (200, 200) (vertical distance = 100).
      final TestGesture gestureV1 = await tester.startGesture(
        const Offset(100, 100),
      );
      final TestGesture gestureV2 = await tester.startGesture(
        const Offset(200, 200),
      );
      await tester.pump();
  ...
  testWidgets(
    'PinchToZoomArea updates ScrollConfiguration physics to NeverScrollableScrollPhysics when 2 or more pointers are down',
    (tester) async {
      final store = await inMemoryKeyValueStore();
            ...
            home: Scaffold(
              body: PinchToZoomArea(
                child: SizedBox.expand(
                  child: Builder(
                    builder: (context) {
                      activePhysics = ScrollConfiguration.of(
                        context,
  ...
  testWidgets(
    'PinchToZoomArea cancels ongoing scroll drag when 2 or more pointers are down',
    (tester) async {
      final store = await inMemoryKeyValueStore();
